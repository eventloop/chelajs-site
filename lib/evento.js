var yaml = require('js-yaml'),
    marked = require('marked'),
    util = require('util'),
    Meethub = require('meethub');

var Markup = function (string) {
  this.klass = 'Markup';
  this.html = marked(string);
  this.md = string;
};

var MarkupType = new yaml.Type('!markdown', {
  kind: 'scalar',
  resolve: function (data) {
    return data !== null;
  },
  construct: function (string) {
    return new Markup(string);
  },
  instanceOf: Markup,
  represent: function (markup) {
    return markup.markdown;
  }
});

var MARKUP_SCHEMA = yaml.Schema.create([MarkupType]);
var yaml_opts = {schema: MARKUP_SCHEMA};

var meses = 'ene feb mar abr may jun jul ago sep oct nov dic'.split(' ');
var str_date = function (time) {
  return [
    ('0'+time.getDate()).slice(-2),
    meses[time.getMonth()]
  ].join('/');
};

var date_pattern = /^(starts|ends):\s?(.+)$/mg;
var date_replacer = function (_, prop, date) {
  date = new Date(date);
  return prop+': '+date.toISOString();
};

var str_time = function (time) {
  return [
    ('0'+ time.getHours()).slice(-2),
    time.getMinutes()
  ].join(':');
};

var ChelaJS = {
  decorate: function (evt) {
    var time = typeof evt.starts === 'number' ? new Date(evt.starts) : evt.starts;
    evt.string_date = str_date(time);
    evt.string_time = str_time(time);
    return evt;
  },
  unserialize: function (string) {
    return yaml.load(
      string.replace(date_pattern, date_replacer),
      yaml_opts
    );
  },
  serialize: function (evt) {
    return yaml.dump(evt.props, yaml_opts);
  },
  description: function (evt) {
    var comps = [
      evt.intro.html,
      '<p>---</p>',
      evt.talks.map(function (talk) {
        var bio = talk.speaker.bio;
        bio = bio.md || bio;
        var summary = talk.summary;
        summary = summary.md || summary;
        return marked(["",
          "**"+talk.title+"**",
          summary,
          "["+talk.speaker.name+"]("+talk.speaker.url+") | "+bio
        ].join("\n\n"));
      }).join("\n<p></p><p></p>\n\n"),
      '<p>---</p>',
      evt.footer.html
    ];
    return comps.join("\n<p></p>\n");
  }
};

module.exports = ChelaJS;